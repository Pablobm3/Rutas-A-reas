...
Imagen Imagen::ExtraeImagen(int posi, int posj, int dimi, int dimj) {
    ...
    if(col_sup+anchura > nc || fil_sup+altura>nf){
        throw std::invalid_argument("Invalid argument");
    }

    Imagen subimagen(dimi,dimj);

    for(int i=fil_sup ; i < fil_sup + dimi ; i++)
    {

        for(int j= col_sup ; j < col_sup + dimj ; j++){

            Pixel coger = data[i][j];
            subimagen(fil_new,col_new)=coger;
            col_new++;
        }
        fil_new++;
        col_new=0;
